...
def _append_to_scholar_file(entry):
	"""Append a single author entry to the persistent scholar results file."""
	if os.path.exists(SCHOLAR_JSON_PATH):
		with open(SCHOLAR_JSON_PATH, encoding='utf-8') as f:
			scholar_results = json.load(f)
	else:
		scholar_results = []
	scholar_results = [
		existing for existing in scholar_results
		if make_author_id(existing['name'], existing.get('institution', ''))
		!= make_author_id(entry['name'], entry.get('institution', ''))
	]
	scholar_results.append(entry)
...
def scrape_and_score_scholar_author(
	...
	"""Scrape a Google Scholar profile, score the author, persist to disk, and add to the in-memory index."""
